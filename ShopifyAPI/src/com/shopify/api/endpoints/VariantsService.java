...
@EndPoint("")
@ContextPath("/admin/Variants")
//@ResponseHandler(ShopifyResponseHandler.class)
//@Param(name = "Content-type", value = "application/json", dest = HEADER)
public interface VariantsService extends BaseShopifyService {
    ...
    @Path(".json?{0}")
    Variant[] getVariants(String queryParams);

    @Path("/{0}.json")
    Variant getVariant(int id);

    @Path("/{0}.json?{1}")
    Variant getVariant(int id, String queryParams);
